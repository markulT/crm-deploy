        ...
        <div className="bg-gray-800 relative rounded-xl mt-4" onClick={() => {
           ...
           <h3 className="text-lg text-gray-300">Активация:</h3>
            ...
            <h3 className="text-xl ">
                {submitActive ?
                    <div className={'flex'}>
                        <h3 className={'absolute z-20 right-40  bottom-3 cursor-pointer bg-green-500 p-2 rounded-2xl'} onClick={()=>{deleteUser(client._id)}} >Подтвердить</h3>
                        <h3 className={'absolute z-20 right-3 bottom-3 px-2 p-2 cursor-pointer bg-red-600 rounded-2xl'} onClick={()=>{setSubmitActive(false)}}>Отменить</h3>
                    </div>
                    :
                <BiTrash className="absolute right-3 bottom-3 text-red-700 text-3xl cursor-pointer" onClick={() => {setSubmitActive(true)}}/>}
            </h3>
        </div>
        </div>
        </div>
